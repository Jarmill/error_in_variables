function [p_psatz, cons_psatz, Gram, Coef] = Full_SS_psatz(p, C, d, vars)
...
vect_0 = recovermonoms(pow0, vars);     % monomial
l_0 = length(vect_0);
...
if mod(d_g,2) == 0     % decide degree of poly
    [powi, ~] = momentPowers(0, n_var, floor((2*d-d_g)/2));
else
    [powi, ~] = momentPowers(0, n_var, floor((2*d-d_g-1)/2));
end
vect_i = recovermonoms(powi, vars);                         % monomial
l_i = length(vect_i);
for i = 1:n_g      
...
% generate phi
[pow_phi, ~] = momentPowers(0, n_var, 2*d-d_h);
vect_phi = recovermonoms(pow_phi, vars);            % monomial
