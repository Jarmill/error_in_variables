...
for i = 1:length(sim_eps)
    for k = 1:length(sim_eps{i})
        ...
        T = sys_curr.T;
        out = Dual_SS_Extended(sys_curr, d, sys_curr.T, type, obj);
        sol = optimize(out.cons, out.obj, opts);

        if sol.problem==0
            success_eps(i) = success_eps(i) + 1;
        end
        fprintf('%d: \t\t eps %d out of %d: %d\n', i, k, length(sim_eps{i}), sol.problem)
    end    
    save('extended_experiment_stab_std.mat', 'success_eps');
...
for i = 1:length(sim_time)
    for k = 1:length(sim_time{i})
        ...
        fprintf('%d: \t\t eps %d out of %d: %d\n', i, k, length(sim_time{i}), sol.problem)
    end
    save('extended_experiment_stab_std.mat', 'success_eps', 'success_T');    
    fprintf('eps %d out of %d\n', i, length(sim_time))
end