function out = Dual_SS_all_noise_manual(sim, d, T, obj)
% same as Dual_SS_manual except line 40

% extract data and size
sim.X_noise = sim.X_noise(:,1:T);   % state data
sim.U_noise = sim.U_noise(:,1:T-1);             % input data
tol = sim.tolerance;                % delta in paper
n = size(sim.X_noise,1);            % dim of state
m = size(sim.U_noise,1);                  % dim of input
...
for i = 1:2*n^2+n       
    cons = Dual_SS_psatz_all_noise_manual(sim, cP(:,i), d, n, m, T);   
    Cons = [Cons; cons];
end
out.cons = Cons;
out.K = K;
